...
export const EditListapre = () => {
    ...
    console.log('Fila actualizada:', updatedProduct);
     ...
     // const columnIdentifiers = ['codigo_producto', 'nit', 'precio']; //Identificadores válidos para le backend
      ...
      const productos = priceList.map((row, rowIndex) => {
        const { codigo, descipcion, precio } = row;
        if (!codigo || !descipcion || !precio) {
          hasEmptyFields = true;
          setEmptyRows((prevEmptyRows: number[]) => [...prevEmptyRows, rowIndex]);
          return { codigo, descipcion, precio, isEmpty: true };
        }
        return row;
      });
      ...
      if (id) {
        ...
        updateLP(payload, id)
          .then(() => {
            messageApi.open({
              type: "success",
              content: `Lista precios modificada con exito!`,
            });
            setTimeout(() => {
              navigate(-1);
            }, 800);
      ...
      render: (text: string, record: any, index: number) =>
        editingIndex === index ? (
          <Input
            ...
            onChange={(e) => {
              const updatedProduct = { ...record, descripcion: e.target.value };
              const updatedList = [...priceList];
              updatedList.splice(index, 1, updatedProduct);
              setPriceList(updatedList);
            }}
  ...
  const handleAddLine = () => {
    ...
    if (lastLine && (lastLine.codigo === '' || lastLine.descipcion === '' || lastLine.precio === '')) {
  ...
  return (
    ...
    <StyledCard>
        ...
        <>
          <Typography.Title level={4}>EditaAAr Lista de Pqweqrecios</Typography.Title>
          <Divider />
