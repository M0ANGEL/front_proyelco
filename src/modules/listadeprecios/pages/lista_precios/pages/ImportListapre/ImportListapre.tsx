...
import { getTerceros } from "@/services/admin-terceros/tercerosAPI";
import { getNitTercero, getNitTerceroProv } from "@/services/salud/conveniosTipoAPI";
import { SearchBar } from "../ListaPrecios/styled";
...
export const ImportListapre = () => {
  ...
  const [motivosAuditoria, setMotivosAuditoria] = useState<SelectProps["options"]>([]);
  ...
  const filterOptionBod = (input: string, option?: { label: string; value: string }) =>
    (option?.label ?? '').toLowerCase().includes(input.toLowerCase());

  const handleSelectedBodegasChange = (selectedBodegas) => {
    setSelectedBodegasForm(selectedBodegas);
  };

  const handleChangePagination: PaginationProps["onChange"] = (page) => {
    // setLoaderTable(true);
    fetchNitTercero(value, page);
  };
  ...
  const handleFileChange = (info) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        console.log("array excel", productos)
        ...
        //     const { codigo, nit, precio } = producto;
      ...
      >
        ...
        >
          <Form layout="vertical" form={form} onFinish={handleSave}>
            ...
            <Row gutter={24}>
              <Col span={12}>
                <StyledFormItem required label="Nit Proveedor:">
