...
//               // Verificar si es nivel 2 y tiene hijos
//               if (item.nivel === 2) {
...
//     // Si es un hijo (nivel 3), verificar si el padre también debe actualizarse
//     if (record.nivel === 3) {
//       // Buscar el padre en los bloques
//       bloques.forEach((bloque) => {
//         const padre = bloque.items.find(
//           (item) => item.nivel === 2 && item.descripcion === record.padre
//         );
//         if (padre) {
//           // Puedes agregar lógica aquí si necesitas actualizar algo en el padre
//           console.log(`Hijo ${record.id} actualizado, padre: ${padre.id}`);
//         }
//       });
//     }
//   };
...
//     try {
//       // Obtener todos los padres (nivel 2) que tienen al menos un hijo seleccionado
//       const padresConHijosSeleccionados: number[] = [];

//       // Buscar padres que tengan al menos un hijo seleccionado
//       Object.keys(seleccionadosHijos).forEach((padreId) => {
//         const idPadre = parseInt(padreId);
//         const hijos = seleccionadosHijos[idPadre];

//         if (
//           hijos &&
//           Object.values(hijos).some((seleccionado) => seleccionado)
//         ) {
//           padresConHijosSeleccionados.push(idPadre);
//         }
//       });
...
//       // 1. Primero procesar padres de nivel 2 que tienen hijos seleccionados
//       padresConHijosSeleccionados.forEach((padreId) => {
//         // Buscar el padre en los bloques
//         bloques.forEach((bloque) => {
//           const padre = bloque.items.find((p) => p.id === padreId);
//           if (padre && padre.nivel === 2) {
//             // Agregar los hijos seleccionados de este padre
//             const hijosSeleccionados = Object.keys(
//               seleccionadosHijos[padreId] || {}
//             ).filter((hijoId) => seleccionadosHijos[padreId][parseInt(hijoId)]);

//             const subHijosArray: any[] = [];

//             hijosSeleccionados.forEach((hijoId) => {
//               const hijo = bloque.items.find((h) => h.id === parseInt(hijoId));
//               if (hijo && hijo.nivel === 3) {
//                 subHijosArray.push({
//                   id: hijo.id,
...
//                   um: hijo.um || "",
//                   cant_total: hijo.cant_total || "0",
//                   valor_sin_iva: hijo.valor_sin_iva || "0",
//                   tipo_insumo: hijo.tipo_insumo || "",
//                   agrupacion: hijo.agrupacion || "",
//                   nivel: hijo.nivel,
//                   es_padre: false,
//                   id_padre: padreId,
//                   nombre_padre: padre.descripcion || "",
//                 });
//               }
//             });

//             // Agregar el padre con sus hijos SOLO si tiene hijos seleccionados
//             if (subHijosArray.length > 0) {
//               datosAEnviar.push({
//                 id: padre.id,
//                 cantidad: cantidades[padre.id] || 0,
//                 padre: padre.padre || "",
//                 codigo: padre.codigo || "",
//                 codigo_proyecto: codigo_proyecto,
//                 descripcion: padre.descripcion || "",
//                 um: padre.um || "",
//                 cant_total: padre.cant_total || "0",
//                 valor_sin_iva: padre.valor_sin_iva || "0",
//                 tipo_insumo: padre.tipo_insumo || "",
//                 agrupacion: padre.agrupacion || "",
//                 nivel: padre.nivel,
//                 es_padre: true,
//                 tiene_hijos_seleccionados: true,
//                 hijos_seleccionados: subHijosArray.length,
//                 subHijos: subHijosArray,
//               });
//             }
//           }
//         });
...
//       // 2. Procesar padres de nivel 2 sin hijos que tengan el checkbox seleccionado
//       Object.keys(seleccionados).forEach((padreId) => {
//         const idPadre = parseInt(padreId);

//         // Verificar si este padre ya fue procesado (tiene hijos seleccionados)
//         if (padresConHijosSeleccionados.includes(idPadre)) {
//           return; // Ya fue procesado arriba
//         }

//         // Si el checkbox del padre está seleccionado
//         if (seleccionados[idPadre]) {
//           bloques.forEach((bloque) => {
//             const padre = bloque.items.find((p) => p.id === idPadre);
//             if (padre && padre.nivel === 2) {
//               // Verificar si realmente tiene hijos
//               const tieneHijos = bloque.items.some(
//                 (item) => item.nivel === 3 && item.padre === padre.descripcion
//               );

//               if (!tieneHijos) {
//                 // Es un padre sin hijos, agregarlo sin subHijos
//                 datosAEnviar.push({
//                   id: padre.id,
//                   cantidad: cantidades[padre.id] || 0,
//                   padre: padre.padre || "",
...
//                   nivel: padre.nivel,
//                   es_padre: true,
//                   tiene_hijos_seleccionados: false,
//                   hijos_seleccionados: 0,
//                   // No incluir subHijos si no hay hijos
//                 });
//               }
//             }
//           });
...
//       // Verificar si hay elementos con cantidad mayor a 0
//       const elementosConCantidad = datosAEnviar.filter(
//         (item) =>
//           item.cantidad > 0 ||
//           (item.subHijos &&
//             item.subHijos.some((hijo: any) => hijo.cantidad > 0))
//       );

//       if (elementosConCantidad.length === 0) {
//         message.warning(
//           "No hay elementos con cantidad mayor a 0. Ingrese cantidades válidas."
//         );
//         setGuardando(false);
//         return;
//       }
...
//       // Llamar a la API - ENVIAR DIRECTAMENTE EL ARRAY
...
//         if (totalHijos > 0) {
//           const padresConHijos = datosAEnviar.filter(
//             (item) => item.es_padre && item.subHijos && item.subHijos.length > 0
//           );
...
//     const mostrarCheckbox = tieneHijos;
...
//       //     <InputNumber
//       //       min={0}
//       //       max={parseFloat(record.cant_total || "0")}
...
//       //       disabled={!seleccionadosHijos[item.id]?.[record.id]}
//       //     />
//       //   ),
//       // },
//       {
...
//             {parseFloat(record.cant_total || "0").toFixed(2)}
//           </Text>
//         ),
//       },
//     ];
...
//                       <li>
//                         Los items de{" "}
//                         <strong>nivel 2 que tienen hijos (nivel 3)</strong>{" "}
//                         muestran un checkbox "Solicitar"
//                       </li>
//                       <li>
//                         <strong>Regla importante:</strong> Si selecciona al
//                         menos 1 hijo, automáticamente se incluirá el padre en la
//                         solicitud
//                       </li>
//                       <li>
//                         Los padres sin hijos seleccionados se pueden marcar
//                         individualmente
//                       </li>
//                       <li>
...
//                         tengan el checkbox "Solicitar" marcado y una cantidad
//                         mayor a 0
//                       </li>
//                     </ul>
//                   </>
