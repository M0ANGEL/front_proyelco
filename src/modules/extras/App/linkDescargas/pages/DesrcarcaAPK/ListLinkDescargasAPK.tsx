...
import { BASE_URL, KEY_ROL } from "@/config/api";
import dayjs from "dayjs";
import {
  DeleteMaLink,
  getLinkDescargas,
} from "@/services/marcaciones-asistencias/ma_linkDescargasApi";
...
export const ListLinkDescargasAPK = () => {
    ...
    getLinkDescargas().then(({ data: { data } }) => {
      const categorias = data.map((categoria) => ({
        ...
        created_at: categoria?.created_at
          ? dayjs(categoria?.created_at).format("DD-MM-YYYY HH:mm")
          : "nada",
      }));
    ...
    <StyledCard
      ...
      extra={
        <Link to={`${location.pathname}/create`}>
          <Button
            type="primary"
            style={{ display: user_rol === "administrador" ? "block" : "none" }}
          >
            Crear
          </Button>
        </Link>
