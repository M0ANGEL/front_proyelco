...
export const ListaActividadesCelsiaProyecto = () => {
  ...
  // Función para determinar el color de la fecha real
  ...
  // Función para renderizar la fecha real con tooltip
  ...
  // Calcular fechas de entrega del proyecto
  const calcularFechasEntrega = () => {
    if (data.length === 0) return { esperada: "N/A", real: "N/A" };

    // La fecha de entrega esperada es la fecha_proyeccion de la última actividad (orden mayor)
    const ultimaActividad = [...data].sort((a, b) => b.orden - a.orden)[0];
    const fechaEsperada = ultimaActividad?.fecha_proyeccion
      ? dayjs(ultimaActividad.fecha_proyeccion).format("DD/MM/YYYY")
      : "N/A";

    // La fecha de entrega real es la fecha_confirmacion de la última actividad si está completada
    const ultimaCompletada = [...data]
      .filter((a) => a.estado == "2" && a.fecha_confirmacion)
      .sort((a, b) => b.orden - a.orden)[0];

    const fechaReal = ultimaCompletada?.fecha_confirmacion
      ? dayjs(ultimaCompletada.fecha_confirmacion).format("DD/MM/YYYY")
      : "Pendiente";

    return { esperada: fechaEsperada, real: fechaReal };
  };
