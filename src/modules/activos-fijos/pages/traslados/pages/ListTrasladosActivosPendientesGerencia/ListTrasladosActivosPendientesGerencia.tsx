...
import {
  aceptarTrasladoGerencia,
  getListaTrasladosActivosPendientesAdminGerencia,
} from "@/services/activos/trasladosActivosAPI";
import { Content } from "antd/es/layout/layout";
import { fetchUserProfile } from "@/services/auth/authAPI";

export const ListTrasladosActivosPendientesGerencia = () => {
  ...
  const fetchDocumentos = () => {
    const estado = "aceptadoAdmin";
    getListaTrasladosActivosPendientesAdminGerencia(estado)
      .then(({ data }) => {
  ...
  const handleAccept = async (key: React.Key) => {
    ...
    aceptarTrasladoGerencia(id_activo,userId)
      .then((response) => {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      key: "activo",
      render: (record: any) =>
        `${record.id} - ${record.nombre || "Sin nombre"}`,
    },
    {
