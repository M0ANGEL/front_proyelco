...
export const ListMovimientosAmeceres = () => {
  const [activosAmeceres, setActivosAmeceres] = useState<
    Activos[]
  >([]);
  ...
  useEffect(() => {
    fetchActivosAmeceres();
  }, [currentPage]);

  useEffect(() => {
    applyFilters();
  }, [activosAmeceres, dateRange, searchInput]);

  const applyFilters = () => {
    let filtered = activosAmeceres;
  ...
  // Cargar Activos Ameceres
const fetchActivosAmeceres = async () => {
  setLoaderTable(true);
  try {
    const categoria = "MUEBLES Y ENSERES";
    const response = await getListaActivosSubCategoria(categoria);
    setActivosAmeceres(response.data);
    setPagination({
      total: response.total,
      per_page: response.per_page,
    });
  } catch (error: any) {
    const errorMessage = error.response?.data?.message || "Error al cargar los ameceres.";
  ...
  const handleFileChangeImages = async (
    ...
    try {
      ...
      if (response.data?.status === "success") {
        ...
        fetchActivosAmeceres(); // Refrescar la lista de mantenimientos o archivos
      } else {
        notification.open({
          type: "error",
  ...
  const handleViewDetails = async (id: number) => {
    ...
    try {
      ...
      if (response && response.data.data.datos) {
        ...
        // const ActivosMap = activosAmeceres.reduce(
  ...
  const handleSaveChanges = async (record: Activos) => {
    try {
      ...
      setActivosAmeceres((prevActivos) =>
  ...
  const columns: ColumnsType<Activos> = [
    ...
    },
    {
        ...
        render: (bodega_info: any, record: Activos) => (
          <Select
            value={selectedLocation[record.id] ?? bodega_info.bod_nombre} 
            style={{ width: 200 }}
            showSearch 
  ...
  return (
    <Layout>
      ...
      <StyledCard title="Movimientos Ameceres">
