...
export const ListMovimientosEquiposComputo = () => {
  const [activosEquiposComputo, setActivosEquiposComputos] = useState<
    Activos[]
  >([]);
  ...
  useEffect(() => {
    fetchActivosEquiposComputo();
  }, [currentPage]);

  useEffect(() => {
    applyFilters();
  }, [activosEquiposComputo, dateRange, searchInput]);

  const applyFilters = () => {
    let filtered = activosEquiposComputo;
...
// Cargar Activos Equipos de Cómputo
const fetchActivosEquiposComputo = async () => {
  setLoaderTable(true);
  try {
    const categoria = "EQUIPO DE COMPUTO Y COMUNICACIÓN";
    const response = await getListaActivosSubCategoria(categoria);
    setActivosEquiposComputos(response.data);
    setPagination({
      total: response.total,
      per_page: response.per_page,
    });
  } catch (error: any) {
    const errorMessage = error.response?.data?.message || "Error al cargar los equipos de cómputo.";
  ...
  const handleFileChangeImages = async (
    ...
    try {
      ...
      if (response.data?.status === "success") {
        ...
        fetchActivosEquiposComputo(); // Refrescar la lista de mantenimientos o archivos
      } else {
        notification.open({
          type: "error",
  ...
  const handleViewDetails = async (id: number) => {
    ...
    try {
      ...
      if (response && response.data.data.datos) {
        ...
        // const ActivosMap = activosEquiposComputo.reduce(
  ...
  ?? (record.usuarios ? Object.values(record.usuarios).map((user: { id: number }) => user.id) : []);
      ...
      setActivosEquiposComputos((prevActivos) =>
        ...
        return (
          <Select
            ...
            options={users.map((user) => ({
              label: user.nombre, // Mostrar nombre
              value: user.id, // ID como valor
            }))}
          />
        );
      }
  ...
  return (
    <Layout>
      <StyledCard title="Movimientos Equipos De Computo">
