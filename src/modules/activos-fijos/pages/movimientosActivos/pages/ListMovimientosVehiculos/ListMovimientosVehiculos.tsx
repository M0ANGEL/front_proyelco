...
export const ListMovimientosVehiculos = () => {
  const [activosVehiculos, setActivosVehiculos] = useState<
    Activos[]
  >([]);
  ...
  useEffect(() => {
    fetchActivosVehiculos();
  }, [currentPage]);

  useEffect(() => {
    applyFilters();
  }, [activosVehiculos, dateRange, searchInput]);

  const applyFilters = () => {
    let filtered = activosVehiculos;
  ...
  // Cargar Activos Vehículos
const fetchActivosVehiculos = async () => {
  setLoaderTable(true);
  try {
    const categoria = "EQUIPO DE TRANSPORTE";
    const response = await getListaActivosSubCategoria(categoria);
    setActivosVehiculos(response.data);
    setPagination({
  ...
  const handleFileChangeImages = async (
    ...
    try {
      ...
      if (response.data?.status === "success") {
        ...
        fetchActivosVehiculos(); // Refrescar la lista de mantenimientos o archivos
      } else {
        notification.open({
          type: "error",
  ...
  const handleViewDetails = async (id: number) => {
    ...
    try {
      ...
      if (response && response.data.data.datos) {
        ...
        // const ActivosMap = activosVehiculos.reduce(
  ...
  const handleSaveChanges = async (record: Activos) => {
    try {
      ...
      setActivosVehiculos((prevActivos) =>
  ...
  return (
    <Layout>
      <StyledCard title="Movimientos Vehiculos">
