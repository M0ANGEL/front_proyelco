...
export const ListMovimientosPropiedades = () => {
  const [activosPropiedades, setActivosPropiedades] = useState<
    Activos[]
  >([]);
  ...
  useEffect(() => {
    applyFilters();
  }, [activosPropiedades, dateRange, searchInput]);

  const applyFilters = () => {
    let filtered = activosPropiedades;
  ...
  const fetchPropiedades = async () => {
    ...
    try {
      ...
      setActivosPropiedades(response.data);
      setPagination({
        total: response.total,
        per_page: response.per_page,
      });
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || "Error al cargar las propiedades.";
  ...
  const handleFileChangeImages = async (
    ...
    try {
      ...
      if (response.data?.status === "success") {
        ...
        fetchPropiedades(); // Refrescar la lista de mantenimientos o archivos
      } else {
        notification.open({
          type: "error",
  ...
  const handleViewDetails = async (id: number) => {
    ...
    try {
      ...
      if (response && response.data.data.datos) {
        ...
        // const ActivosMap = activosPropiedades.reduce(
  ...
  const handleSaveChanges = async (record: Activos) => {
    try {
      ...
      setActivosPropiedades((prevActivos) =>
  ...
  return (
    <Layout>
      <StyledCard title="Movimientos Maquina y Equipo">
