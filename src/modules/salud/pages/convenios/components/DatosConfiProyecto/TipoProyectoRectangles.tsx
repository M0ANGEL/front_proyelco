...
//                   render={({ field }) => (
//                     <StyledFormItem required label="Nombre de validacion requerida  para iniciar con esta actividad:">
//                       <TextArea
//                         {...field}
...
export const TipoProyectoRectangles = ({ tipos, onChangeTipos }: Props) => {
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const newDest = [...destList];
      const [moved] = newSource.splice(source.index, 1);
      newDest.splice(destination.index, 0, { ...moved, requiereValidacion: undefined });
      ...
      {(providedDraggable) => (
        ...
        >
          ...
          >
            {isConfirmado && (
              <>
                <StyledFormItem required label="¿Este proceso requiere validación?">
                  <Radio.Group
                    onChange={(e) =>
                      actualizarCampo(index, "requiereValidacion", e.target.value)
                    }
                ...
                {tipo.requiereValidacion === "si" && (
                  <Controller
                    ...
                    control={methods.control}
                    rules={{
                      required: "Este campo es obligatorio si se seleccionó 'Sí'",
                    }}
                    render={({ field, fieldState: { error } }) => (
                      <StyledFormItem
                        required
