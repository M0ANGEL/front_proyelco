...
export const ReporteAsistenciaTH = () => {
  ...
  const obtenerValoresUnicos = (data: DataType[]) => {
    ...
    const contratistas = [...new Set(data.map(item => item.nombre_contratista || "No asignado"))];
    const obras = [...new Set(data.map(item => item.nombre_obra || "Sin obra"))];
  ...
  const aplicarFiltros = (data: DataType[] = initialData) => {
    ...
    if (filtrosAvanzados.contratista.length > 0) {
      filtered = filtered.filter(item => 
        filtrosAvanzados.contratista.includes(item.nombre_contratista || "No asignado")
      );
    }
  ...
  const handleFiltroChange = (tipo: keyof FiltrosAvanzados, values: string[]) => {
    const nuevosFiltros = {
      ...filtrosAvanzados,
      [tipo]: values,
    };
  ...
  const filtrosMenu = (
    <Menu style={{ padding: 16, width: 300 }}>
      ...
      <Space style={{ width: '100%', justifyContent: 'space-between' }}>
        <Button 
          size="small" 
          onClick={() => setShowFiltrosAvanzados(false)}
        >
          Cerrar
        </Button>
        <Button 
              ...
              <Space>
                {filtrosActivos.map(filtro => (
                  <Tag 
                    key={filtro} 
                    closable 
                    onClose={() => limpiarFiltro(filtro as keyof FiltrosAvanzados)}
                    color="blue"
                  >
                    {filtro === 'estado' ? 'Estado' : 
                     filtro === 'contratista' ? 'Contratista' : 
                     filtro === 'obra' ? 'Obra' : 'Cargo'}
                  </Tag>
                ))}
