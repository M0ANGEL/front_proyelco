...
import { getExcel, getListaFVC, getPDF } from "@/services/documentos/fvcAPI";
...
export const ListarDocumentos = ({ privilegios, tab, setLoader }: Props) => {
  ...
  const generarExcel = (key: React.Key) => {
    ...
    getExcel(key.toString())
      .then(({ data }) => {
        fileDownload(data, "FACTURA CONCEPTO.xlsx");
        notificationApi.open({
          type: "success",
          message: "Documento generado con exito!",
        });
  ...
  const fetchDocumentos = (query = "", page = 1) => {
    ...
    getListaFVC(page, getSessionVariable(KEY_BODEGA), estado, query)
      .then(({ data: { data } }) => {
        console.log(data);
