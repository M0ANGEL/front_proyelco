...
import { getListaDRV, getPDF } from "@/services/documentos/drvAPI";
...
export const ListarDocumentos = ({ privilegios, tab, setLoader }: Props) => {
  ...
  const fetchDocumentos = (query = "", page = 1) => {
    ...
    getListaDRV(page, getSessionVariable(KEY_BODEGA), estado, query)
      .then(({ data: { data } }) => {
        setPagination(data);
        const documentos: DataType[] = data.data.map((item) => {
          return {
            key: item.id,
            consecutivo_remision: item.rvd_cabecera.consecutivo,
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {
      title: "Consecutivo Remision",
      dataIndex: "consecutivo_remision",
      key: "consecutivo_remision",
      sorter: true,
      align: "center",
      width: 120,
    },
