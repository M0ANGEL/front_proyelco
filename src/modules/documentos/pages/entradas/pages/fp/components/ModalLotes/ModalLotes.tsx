...
export const ModalLotes = ({
  ...
  useEffect(() => {
    ...
    if (producto) {
      setProductoInfo({
        ...
        cant_pedida: producto.total_cantidad,
  ...
  const addLote = (
    ...
    const lotes: DataTypeChildren[] = values.lotes.map((item, index) => {
      const f_vencimiento = dayjs(item.fecha_vencimiento);
      const lote_key = `${producto_id}_${item.lote}_${f_vencimiento.format(
        "DD-MM-YYYY"
      )}`;
      // Se valida que los lotes digitados no estén repetidos dentro del mismo arreglo
      if (
        values.lotes.some(
          (lote, indexLote) =>
            indexLote != index &&
            dayjs(lote.fecha_vencimiento).format("DD-MM-YYYY") ==
              f_vencimiento.format("DD-MM-YYYY") &&
            item.lote == lote.lote
        )
      ) {
        flagDuplicado = true;
    ...
    const newData = dataSource.map((producto) => {
      if (producto.key == producto_id) {
        return {
          ...producto,
          total_ingreso: producto.total_ingreso + cantIngresoLotes,
      ...
      <Modal
        ...
        title={
          ...
          >
            ...
            <Space>
              <Text>
                Cant. por Ingresar: <Tag>{productoInfo?.cant_pedida}</Tag>
              </Text>
              <Text>
                Cant. a Ingresar:{" "}
                <Tag
                  color={
                    productoInfo?.cant_ingresada > productoInfo?.cant_pedida
                      ? "red"
                      : "default"
                  }
                >
                  {productoInfo?.cant_ingresada}
                </Tag>
              </Text>
            </Space>
          </Space>
