  ...
  useEffect(() => {
    if (oldItem) {
      buscarProductosxCodigoPadre(oldItem.cod_padre)
        .then(({ data: { data } }) => {
          console.log(oldItem.laboratorio,data);
          const productos: any[] = data.filter((producto) => producto.descripcion.includes("(UNIDOSIS)") && producto.laboratorio == oldItem.laboratorio )
          .map((producto) => {
            return {
