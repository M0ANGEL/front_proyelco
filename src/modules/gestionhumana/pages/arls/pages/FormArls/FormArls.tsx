...
import { store, edit, update } from "@/services/gestion-humana/arlAPI"
import useSessionStorage from "@/modules/common/hooks/useSessionStorage"
import { KEY_ROL } from "@/config/api"

const { Text } = Typography

export const FormArls = () => {
  const [api, contextHolder] = notification.useNotification()
  const [loader, setLoader] = useState<boolean>(false)
  const control = useForm()
  const [arl, setArl] = useState<Arl>()
  ...
  const onFinish: SubmitHandler<any> = async (data) => {
    ...
    if (arl) {
      update(data, id).then(() => {
        pushNotification({ title: "ARL actualizado con exito!" });
        setTimeout(() => {
          navigate("..")
        }, 800)
      })
      ...
      store(data).then(() => {
        pushNotification({ title: "ARL creada con exito!" });
        setTimeout(() => {
          navigate(-1)
        }, 800)
      }).catch((error) => {
