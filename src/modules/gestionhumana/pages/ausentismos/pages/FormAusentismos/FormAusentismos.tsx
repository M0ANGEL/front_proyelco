...
import { crearAusentismo, getAusentismo, updateAusentismo } from "@/services/gestion-humana/ausentismosAPI"
import useSessionStorage from "@/modules/common/hooks/useSessionStorage"
import { KEY_ROL } from "@/config/api"

const { Text } = Typography

export const FormAusentismos = () => {
  ...
  const fetchAusentismo = (id: string) => {
    getAusentismo(id).then(({ data }) => {
      setAusentismo(data);
      setLoaderSave(false);
    })
  }
  ...
  const onFinish: SubmitHandler<any> = async (data) => {
    ...
    if (ausentismo) {
      updateAusentismo(data, id).then(() => {
        pushNotification({ title: "Ausentismo actualizado con exito!" });
        setTimeout(() => {
          navigate("..")
        }, 800)
      })
    } else {
      crearAusentismo(data).then(() => {
        pushNotification({ title: "Ausentismo creado con exito!" });
        setTimeout(() => {
          navigate(-1)
        }, 800)
      })
    ...
    {contextHolder}
      ...
      >
        <FormProvider {...control}>
          ...
          >
            <StyledCard
              title={(ausentismo ? "Editar" : "Crear") + " Ausentismos"}
              extra={
                <Space>
