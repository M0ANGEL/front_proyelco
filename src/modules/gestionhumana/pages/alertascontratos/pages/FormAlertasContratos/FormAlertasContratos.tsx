...
import { store, edit, update } from "@/services/gestion-humana/alertasContratosAPI"
import useSessionStorage from "@/modules/common/hooks/useSessionStorage"
import { KEY_ROL } from "@/config/api"

const { Text } = Typography

export const FormAlertasContratos = () => {
  const [api, contextHolder] = notification.useNotification()
  const [loaderSave, setLoaderSave] = useState<boolean>(true)
  const control = useForm()
  const [alertaContrato, setAlertaContrato] = useState<AlertaContrato>()
  ...
  const onFinish: SubmitHandler<any> = async (data) => {
    ...
    if (alertaContrato) {
      update(data, id).then(() => {
        pushNotification({ title: "Alerta tipo contrato actualizada con exito!" });
        setTimeout(() => {
          navigate("..")
        }, 800)
      })
    ...
    store(data).then(() => {
      pushNotification({ title: "Alerta tipo contrato creada con exito!" });
      setTimeout(() => {
        navigate(-1)
      }, 800)
    }).catch((error) => {
    ...
    <>
      ...
      >
        <FormProvider {...control}>
          ...
          >
            <StyledCard
              title={(alertaContrato ? "Editar" : "Crear") + " Alertas por dias tipo de contrato."}
              extra={
                <Space>
