...
import { crearBanco, getBanco, updateBanco } from "@/services/gestion-humana/bancosAPI"
...
export const FormBancos = () => {
  ...
  const onFinish: SubmitHandler<any> = async (data) => {
    ...
    setLoaderSave(true);
    if (banco) {
      updateBanco(data, id).then(() => {
        pushNotification({ title: "Banco actualizado con exito!" });
        setTimeout(() => {
          navigate("..")
        }, 800)
      })
    } else {
      crearBanco(data).then(() => {
        pushNotification({ title: "Banco creado con exito!" });
        setTimeout(() => {
          navigate(-1)
        }, 800)
      }).catch((error) => {
    ...
    <>
      ...
      >
        <FormProvider {...control}>
          ...
          >
            <StyledCard
              title={(banco ? "Editar" : "Crear") + " Banco"}
              extra={
                <Space>
