...
import { Form, Space, Spin, Tabs, Typography } from "antd";
...
import {
  ...
  getAmcliente,
  updateAmCliente,
} from "@/services/clientes/AdministrarClientesApi";
import {
  crearpowerBi,
  getpowerBi,
  updatepowerBi,
...
export const FormInformesPowerBI = () => {
    ...
    if (id) {
      ...
      getpowerBi(id)
        ...
        .catch((error) => {
          ...
          notify.error("Error", "No se pudo cargar la información del cliente");
        })
        .finally(() => {
          setLoaderSave(false);
        });
  ...
  const onFinish: SubmitHandler<any> = async (data) => {
    ...
    try {
      if (isEditMode) {
        await updatepowerBi(data, id!);
        notify.success("Éxito", "Informe actualizado correctamente");
        setTimeout(() => {
          navigate("..");
        }, 800);
      } else {
        await crearpowerBi(data);
        notify.success("Éxito", "Informe creado correctamente");
        setTimeout(() => {
          navigate(-1);
        }, 800);
      }
      ...
      notify.error(
        ...
        error.response?.data?.message ||
          "Ocurrió un error inesperado al guardar el Informe"
      );
    } finally {
      setLoaderSave(false);
    }
  ...
  return (
    ...
    >
      <FormProvider {...control}>
        ...
        >
          <GlobalCard
            title={`${isEditMode ? "Editar" : "Crear"} Informe`}
            extra={
              <Space>
                <SaveButton
                  loading={loaderSave}
                  disabled={loaderSave}
                  text={isEditMode ? "Actualizar Informe" : "Crear Informe"}
                  htmlType="submit"
                  icon={<SaveOutlined />}
                />
                <BackButton
          ...
          >
            {hasErrors && (
              ...
              >
                ⚠️ Faltan campos requeridos por diligenciar o existen errores en
                el formulario
              </Text>
            )}
