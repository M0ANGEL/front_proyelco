...
import { FacturaFVE, ResponseFacturaFVE } from '../types'

export const getListaConsultaDoc = async (data:any): Promise<ResponseFacturaFVE> => {
  let url="";
  data.data.numero_fve === "" || data.data.numero_fve ===undefined ?
    url=`/listar-documentos-cufe`: url= `/listar-x-documento-cufe`;
